...
def vector_collide(orgin_pos, orgin_angle, object_list, kill = False):
    ...
    if hit_objects:
        closest_index = object_distances.index(min(object_distances))
        closest_object = hit_objects[closest_index]
        if kill: closest_object.kill()
        return(closest_object)
    

#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
#::::::::::::::::::: move player out of wall and such :::::::::::::::::

def collidefix(hit_box, stagnent_obj_list, obj_to_move = None):
    if not obj_to_move: obj_to_move = hit_box
    
    for obstacle in stagnent_obj_list:
        if hit_box.colliderect(obstacle.rect):
            # make the smallest change possible to get hit_box out of the obstacle
            delta_left = obstacle.rect.right - hit_box.left
            delta_right = hit_box.right - obstacle.rect.left
            delta_top = obstacle.rect.bottom - hit_box.top
            delta_bottom = hit_box.bottom - obstacle.rect.top
            
            # add all deltas to a list:
            deltas = [delta_left, delta_right, delta_top, delta_bottom]
            
            if   deltas.index(min(deltas)) == 0: obj_to_move.rect.x += min(deltas)
            elif deltas.index(min(deltas)) == 1: obj_to_move.rect.x -= min(deltas)
            elif deltas.index(min(deltas)) == 2: obj_to_move.rect.y += min(deltas)
            elif deltas.index(min(deltas)) == 3: obj_to_move.rect.y -= min(deltas)
...
class Animation():
    ...
    def __init__(self, image, num_frames, duration,
